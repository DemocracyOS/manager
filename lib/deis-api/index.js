...
function configurate(done, deployment) {
  deployment.populate({
    ...
    var _config = env({
      ...
      MONGO_USERS_URL: deployment.mongo_url,
...
function deploy(done, deployment) {
  ...
  deis.builds.create(deployment.name, (registry + image), function(err, app) {
    if (err) {
      log('Error building \'%s\' app: %s', deployment.name, err);
      return done(err);
    }

    log('Deis app built successfully: %s', deployment.name, app);
