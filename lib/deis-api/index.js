...
exports.deploy = function(name, envs, fn) {
  ...
  batch.push(function(done){ setLimits(name, done); });
...
exports._login = function login (done){
...
exports._create = function create (name, done) {
...
exports._destroyIfExists = function destroyIfExists (name, done) {
...
exports._destroy = function destroy (name, done) {
...
exports._configurate = function configurate (name, envs, done) {
...
exports._setLimits = function setLimits (name, done) {
  var limits = {
    memory: { cmd: '512M' },
    // cpu: { cmd: 256 }
  }
  ...
  deis.limits.set(name, limits, function (err, result) {
    if (err) {
      ...
      destroyIfExists(name, function(_err){
        if (_err) log('Found error when trying to set limits on "%s" Deis app: %s', name, _err);
        done(err);
      });
      return;
    }

    log('Limits set to Deis app "%s": %j', name, result);

    done(null);
  });
}

exports._build = function build(name, done) {
