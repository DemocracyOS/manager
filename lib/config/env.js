...
module.exports = {
  ...
  mongoUrl:      env.MONGO_URL, // DB connection for Manager
  deis: { // Deis API auth. Deis server where the Deployments will be instantiated.
    controller: env.DEIS_CONTROLLER,
    username:   env.DEIS_USER,
    password:   env.DEIS_PASSWORD,
  },
  'deployment image':             env.DEPLOYMENT_IMAGE, // The docker image of DemocracyOS/app that will be used for Deployments.
  'deployment internal registry': env.DEPLOYMENT_INTERNAL_REGISTRY, // (optional) Provide an internal registry if the 'deployment image' is not hosted ong Docker's Hub.
  'deployment db server': env.DEPLOYMENT_DB_SERVER, // Mongo server where the DB's for the Deployments will be created. Must provide a user with 'root' role. Accepts simple server or replicaSet string.
  'deployment env': { // ENV vars that will be setted by default on new Deployments.
  ...
  'reserved names': env.RESERVED_NAMES ? env.RESERVED_NAMES.split(',') : [], // Reserved names that can't be used on Deployments
  locale :          env.LOCALE,
  port:             env.PORT,
