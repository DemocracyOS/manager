...
exports.create = function (deployment, fn){
  if (deployment.mongoUrl) return fn('The deployment \'%s\' already has a database attached.', deployment.name);
...
exports.drop = function (deployment, fn){
  ...
  clearDB(deployment.mongoUrl, function(err){
    ...
    compose.deploymentUserDelete({
      ...
      log('Deleted DB user of deployment "%s".', deployment.name);
